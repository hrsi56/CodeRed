...
import type { AtlasData, FatalityEvent } from './types';
import {
  computeCityWeights,
...
export function usePanelData(data: AtlasData | null, range: DateRange | undefined): PanelData {
  return useMemo(() => {
    if (!data || !range?.from) {
      return { cityWeights: [], hourHistogram: EMPTY_HIST, totalAlerts: 0, fatalities: [], ownHeatmapMax: 1, dayCount: 0 };
    }
    ...
    return {
      ...
      fatalities: filterFatalitiesByDate(data.fatalities, isoDay(range.from), isoDay(range.to ?? range.from)),
