...
5. **Secrets live in env vars, never in git.** ACLED credentials → `ACLED_KEY` / `ACLED_EMAIL` (or equivalent). Provide a `.env.example`; add `.env` to `.gitignore`. Never print secrets in logs.
